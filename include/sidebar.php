...
<head>
  ...
  <style>
    ...
    .stat-card {
      ...
      border-top: 3px solid var(--accent, var(--marquee));
      border-radius: .5rem;
    }
    ...
    .badge-action {
      background: rgba(201, 147, 46, .15);
      color: var(--marquee);
    }

    .badge-horror {
      background: rgba(179, 36, 46, .15);
      color: var(--velvet);
    }

    .badge-comedy {
      background: rgba(47, 157, 99, .15);
      color: var(--ok);
    }
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <nav class="nav flex-column p-2 pt-3">
      ...
      <div class="sidebar-footer mt-5">
        <div class="avatar f rounded-circle text-white d-flex align-items-center justify-content-center fw-bold"
          id="userAvatar"
          style="width: 40px; height: 40px; background: linear-gradient(135deg, #b91c1c, #d97706); font-size: 14px; text-transform: uppercase;">
          --
        </div>
        <div>
          <div class="fw-bold text-dark text-truncate" id="userName">Loading...</div>
        ...
        $.ajax({
          ...
          success: function(res) {
            if (res.status === 'success') {
              $('#userAvatar').text(res.data.initials);
